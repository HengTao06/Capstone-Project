...
function render(data) {
    ...
    document.getElementById("hero-img").src =
        data.image ? `../../../assets/images/${data.image}` : "https://placehold.co/800x300";

    document.getElementById("title").innerText = data.name;
    document.getElementById("location").innerText = data.location;
    document.getElementById("description").innerText = data.description || "No description.";

    document.getElementById("rating").innerHTML = `${renderStars(data.rating)} <span class="rating-number">${data.rating}</span>`;
